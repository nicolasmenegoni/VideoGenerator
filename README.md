...
- **Forge WebUI + Civitai** para gerar imagens verticais com IA a partir das frases.
...
Para usar **Gerar imagem**, o app tenta conectar no Forge WebUI local em `http://127.0.0.1:7860`. Se o Forge estiver fechado, ele tenta iniciar o comando configurado em **Comando para iniciar Forge**; se esse campo estiver vazio, tenta instalar o Forge automaticamente na **Pasta de instalação automática do Forge** (baixando/clonando `lllyasviel/stable-diffusion-webui-forge`) e iniciar com `--api`. No primeiro uso, o Forge ainda pode demorar vários minutos para baixar dependências e modelos.
...
1. Abra o app.
2. Na aba **APIs**, informe as chaves do Pexels e do Groq. Para imagens com IA, informe também a URL da API do **Forge WebUI** (padrão `http://127.0.0.1:7860`), o comando opcional para iniciar o Forge automaticamente, a pasta onde o Forge deve ser instalado automaticamente, o token do **Civitai** se o modelo precisar de autenticação, o **Civitai Model Version ID** e, opcionalmente, a pasta de checkpoints do Forge para o app baixar o modelo automaticamente. O app salva localmente as chaves e demais configurações quando você clica em **Salvar chaves**, sincroniza o roteiro, gera/atualiza vídeos ou fecha a janela.
