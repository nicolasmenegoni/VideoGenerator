...
5. Na aba **Video**, copie um link do Pexels e clique em **Colar link** ao lado da frase para aplicar direto da área de transferência; o app mostra um preview pequeno da foto/vídeo quando consegue resolver a miniatura. Use **Editar** se quiser ajustar manualmente, ou deixe vazio para busca automática.
...
8. Na aba **Audio**, configure a automação do app do ChatGPT: atalho para abrir o app, tempo antes da primeira captura, tempo máximo de verificação da resposta, espera depois do menu e tempo extra/máximo de gravação. Não é necessário configurar coordenadas: o app captura a janela do ChatGPT e identifica automaticamente o campo de texto, o botão **Enviar**, os **3 pontinhos** da resposta e a opção **Ler em voz alta**. Depois de enviar a mensagem, ele espera 2 segundos e então verifica a tela a cada 1 segundo até encontrar os **3 pontinhos**. No Windows, durante a gravação o app silencia temporariamente outras sessões de áudio para capturar só a voz do ChatGPT; depois restaura os volumes. A gravação tenta parar automaticamente ao detectar silêncio depois da fala.
